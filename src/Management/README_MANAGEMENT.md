...
The method deserialize() of ObjectSerializer class does not check array type data. We have to add the conditional like below:
```
} elseif ($class === 'object') {
    settype($data, 'array');
    return $data;
} elseif (is_array($data)) { //This is the conditional
    $data = (object)$data;
    return $data;
